...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onAddToCart }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative bg-[#F4F1EA] flex items-center justify-center p-5 sm:p-6 overflow-hidden min-h-[260px] sm:min-h-[280px]">
        ...
        <div className="absolute top-3.5 inset-x-3.5 z-10 flex items-center justify-between">
          <span className="px-2.5 py-0.5 text-[10px] font-medium tracking-widest uppercase bg-white/90 backdrop-blur-xs text-[#59524A] rounded-full border border-[#DDD6CB]">
            {product.volume}
          </span>
          <span className="text-[9px] uppercase tracking-wider font-semibold text-emerald-800 bg-white/90 backdrop-blur-xs px-2.5 py-0.5 rounded-full border border-[#DDD6CB]">
            DISPONIBLE
          </span>
        </div>

        {/* Perfume image with link */}
        <motion.div
          initial={{ opacity: 0, scale: 0.96 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ duration: 0.4 }}
          className="relative z-10 w-full max-w-[190px] aspect-[3/4] flex items-center justify-center"
        >
          <Link
